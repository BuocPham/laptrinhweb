...
-- 7. Lấy ra danh sách người dùng trong đó email người dùng là Gmail và tên người dùng bắt đầu bằng chữ m
SELECT * FROM users WHERE user_email LIKE '%@gmail.com' AND user_name LIKE 'm%' ORDER BY user_name ASC;
-- 8. Lấy ra danh sách người dùng trong đó email người dùng là Gmail và tên người dùng có chứa chữ i và có chiều dài lớn hơn 5
SELECT * FROM users WHERE user_email LIKE '%@gmail.com' AND user_name LIKE '%i%' AND CHAR_LENGTH(user_name) > 5 ORDER BY user_name ;
...
SELECT * FROM users WHERE user_name LIKE '%a%' AND CHAR_LENGTH(user_name) BETWEEN 5 AND 9 AND user_email LIKE '%@gmail.com' AND user_email LIKE '%i%' ORDER BY user_name ASC;
...
SELECT * FROM users WHERE (user_name LIKE '%a%' AND CHAR_LENGTH(user_name) BETWEEN 5 AND 9) OR (user_name LIKE '%i%' AND CHAR_LENGTH(user_name) < 9) OR (user_email LIKE '%@gmail.com' AND user_email LIKE '%i%') ORDER BY user_name ;
